...
int main()
{
    ...
    struct sockaddr_in enderecoServidor;    // Informações sobre o servidor
    struct sockaddr_in enderecoCliente;     // Informações sobre o cliente
    int estado = 0;
    int tamanho;
    ...
    //Criar o descritor do servidor - Criando um ponto de conexão
    socketServidor = socket(AF_INET,        // Domínio : IPv4 
    SOCK_STREAM,                            // Tipo : Duplex, confiavel
    0);                                     // Protocolo : Não descrito (Não definiu se é TCP ou UDP)

    if ( socketServidor == INVALID_SOCKET) {
        perror("Erro de inicializacao do Servidor : ");
        exit(EXIT_FAILURE);
    }
    ...
    // Vinculo do servidor com o descritor do Socket
    estado = bind(socketServidor,               // Descritor do Socket 
    (struct sockaddr *) &enderecoServidor,   // Endereço da estrura de informações do servidor
    sizeof( struct sockaddr) );              // Tamanho da estrura de informações do servidor

    if ( estado == -1) {
        perror("Erro ao iniciar o bind : ");
        exit(EXIT_FAILURE);
    }
    ...
    while(1){ 
        ...
        // Aceitar o cliente
        socketCliente = accept(socketServidor, 
        (struct sockaddr *) &enderecoCliente, 
        &tamanho);

        if ( socketCliente == -1) {
            perror("Erro na aceitacao do cliente : ");
            exit(EXIT_FAILURE);
        }
